...
FROM ge WHERE constituency BETWEEN 'S14000021' AND 'S14000026' AND yr = 2017 ORDER BY r, constituency;

-- 5.You can use SELECT within SELECT to pick out only the winners in Edinburgh.
--   Show the parties that won for each Edinburgh constituency in 2017.
SELECT constituency,party FROM 
(SELECT constituency, party, votes, RANK() OVER (PARTITION BY constituency ORDER BY votes DESC) AS r FROM ge WHERE constituency BETWEEN 'S14000021' AND 'S14000026' AND yr = 2017 ORDER BY r, constituency) x
where x.r=1;
...
(SELECT constituency,party FROM 
	(SELECT constituency, party, votes, RANK() OVER (PARTITION BY constituency ORDER BY votes DESC) AS r
	FROM ge WHERE constituency LIKE 'S%' AND yr  = 2017 ORDER BY r , constituency) x
WHERE x.r=1) y
GROUP BY party;